...
- What the learner says and what the recognition checks are two separate decisions:
  ...
  - Whole-phrase mode - `params.targetWords` holds the complete sentence as a single entry, identical to `params.targetPhrase` (for example `targetWords: ["I have an egg."]`). The game shows the sentence as one chip and passes only when the learner says the whole sentence in order, with no skipped words. Use it when the target itself is a small function word inside the sentence (such as `a` / `an`), where per-word checking would let the learner drop it.
