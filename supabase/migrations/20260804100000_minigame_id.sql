-- Pin each minigame instance to its catalog kind (letter_ordering, word_ordering, …).
-- Gameplay `variant` stays as a label (word_spelling, sentence_building, …).

alter table public.minigame_instances
  add column if not exists minigame_id text;

comment on column public.minigame_instances.minigame_id is
  'Catalog minigame external_id (letter_ordering, word_ordering, speak_aloud, word_matching, letter_drawing).';
