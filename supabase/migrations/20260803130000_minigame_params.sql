-- Per-game minigame parameters.
--
-- Each minigame instance now carries the content parameters of the Unity config
-- data ScriptableObject it launches (LetterOrderingDataSO, SpeakAloudDataSO,
-- WordOrderingDataSO, LetterConnectionLevelConfigSO, LetterPathSO). The catalog
-- minigame entry's `content_fields` metadata declares which keys apply per game.

alter table public.minigame_instances
...
begin
  ...
  insert into public.minigame_instances (
    ...
    params, source_path, source_metadata, updated_by
  )
  select
    (m->>'id')::uuid,
    m->>'key',
