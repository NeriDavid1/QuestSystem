-- Temp step positions used -1-ord which violates CHECK (position >= 0).
-- Use a high positive offset instead to free unique (quest_id, position) slots.
...
begin
  ...
  -- Must stay >= 0 to satisfy quest_steps_position_check.
  update public.quest_steps qs
  set position = 1000000 + sub.ord
  from (
