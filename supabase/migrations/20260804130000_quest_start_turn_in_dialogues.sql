-- Quest-level start + turn-in dialogues (Unity startDialogue / turnInDialogue).
-- Values are dialogue keys (same convention as step payload.dialogue_id).

alter table public.quests
  add column if not exists start_dialogue_id text,
  add column if not exists turn_in_dialogue_id text;
