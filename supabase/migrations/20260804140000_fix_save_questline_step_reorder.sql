-- Fix save_questline: step reorder violated unique (quest_id, position)
-- because ON CONFLICT (id) updates collide mid-statement. Also bump
-- questlines.updated_at so optimistic concurrency has a real version.
...
begin
  ...
  insert into public.quests (
    ...
    summary, wait_for_npc_turn_in, start_dialogue_id, turn_in_dialogue_id,
    status, source_path, source_metadata, created_by, updated_by
  )
  select
    (q->>'id')::uuid,
    v_line_id,
    ...
    nullif(q->>'start_dialogue_id', ''),
    nullif(q->>'turn_in_dialogue_id', ''),
  ...
  on conflict (id) do update set
    ...
    start_dialogue_id = excluded.start_dialogue_id,
    turn_in_dialogue_id = excluded.turn_in_dialogue_id,
    status = excluded.status,
    source_path = excluded.source_path,
    source_metadata = excluded.source_metadata,
    updated_by = excluded.updated_by;

  -- Free unique (quest_id, position) slots before applying final positions.
  update public.quest_steps qs
  set position = -1 - sub.ord
  from (
    select (s->>'id')::uuid as id, (ordinality)::integer as ord
