-- Quest turn-in gating.
--
-- QuestDefinitionSO.waitForNpcTurnIn keeps a quest in CAN_FINISH until an NPC
-- calls FinishQuest (vs. auto-completing when the last objective is done). The
-- editor needs to author this flag so exported quests match the runtime.

alter table public.quests
  add column if not exists wait_for_npc_turn_in boolean not null default false;
